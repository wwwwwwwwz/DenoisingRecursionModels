...
class TinyRecursiveReasoningModel_ACTV1_Inner(nn.Module):
    ...
    def noise_latent(self, latent, scheduler):
        ...
        timesteps = torch.randint(
            low=0, high=scheduler.config.num_train_timesteps,
            size=(batch_size,), device=latent.device, dtype=torch.long
        )

        return scheduler.add_noise(latent, eps, timesteps)

    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1InnerCarry, batch: Dict[str, torch.Tensor], scheduler) -> Tuple[TinyRecursiveReasoningModel_ACTV1InnerCarry, torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        seq_info = dict(
        ...
        if not torch.is_inference_mode_enabled() or self.config.noise_inference:
            z_L = self.noise_latent(z_L, scheduler)
            z_H = self.noise_latent(z_H, scheduler)
...
class TinyRecursiveReasoningModel_ACTV1(nn.Module):
    ...
    def forward(self, carry: TinyRecursiveReasoningModel_ACTV1Carry, batch: Dict[str, torch.Tensor]) -> Tuple[TinyRecursiveReasoningModel_ACTV1Carry, Dict[str, torch.Tensor]]:
        ...
        # Forward inner model
        new_inner_carry, logits, (q_halt_logits, q_continue_logits) = self.inner(new_inner_carry, new_current_data, scheduler=self.scheduler)
