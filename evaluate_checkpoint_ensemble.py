...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "One or more checkpoint paths to ensemble. "
            "Evaluation runs sequentially over these checkpoints while reusing evaluator state."
        ),
    )
    parser.add_argument(
        "--output-dir",
...
def _normalize_checkpoint_list(values: List[str]) -> List[str]:
    checkpoints: List[str] = []
    for value in values:
        for part in value.split(","):
            part = part.strip()
            if part:
                checkpoints.append(part)
...
def _print_metrics(title: str, metrics: Optional[dict]) -> None:
    print(f"\n{title}", flush=True)

    if not metrics:
        print("  No metrics produced.", flush=True)
        return

    for key, value in metrics.items():
        if isinstance(value, dict):
            for inner_key, inner_value in value.items():
                print(f"  {key}/{inner_key}: {inner_value}", flush=True)
        else:
...
def main() -> int:
    ...
    checkpoint_list = _normalize_checkpoint_list(args.checkpoint_list)

    if args.output_dir is not None:
        config.checkpoint_path = args.output_dir
    elif config.checkpoint_path is None:
        first_ckpt_name = os.path.splitext(os.path.basename(checkpoint_list[0]))[0]
        last_ckpt_name = os.path.splitext(os.path.basename(checkpoint_list[-1]))[0]
        config.checkpoint_path = os.path.join(
            "checkpoints",
            "eval_ensemble",
            f"{first_ckpt_name}_to_{last_ckpt_name}",
        )
    ...
    for ensemble_idx, checkpoint_path in enumerate(checkpoint_list):
        step = _infer_step(checkpoint_path)
        if step is None:
            if args.step is not None and len(checkpoint_list) == 1:
                step = args.step
            else:
                raise ValueError(
                    f"Could not infer step from checkpoint path: {checkpoint_path}. "
                    "--step is only supported for a single checkpoint."
                )

        if rank == 0:
            print(
                f"\nEvaluating checkpoint "
                f"{ensemble_idx + 1}/{len(checkpoint_list)}: {checkpoint_path} "
                f"(step={step})"
            )
        ...
        if rank == 0:
            _print_metrics(
                f"Ensemble metrics after checkpoint "
                f"{ensemble_idx + 1}/{len(checkpoint_list)}: {checkpoint_path} "
                f"(step={step})",
                metrics,
            )

        # Free this checkpoint before loading the next one
        del train_state
        del model
    ...
    if rank == 0:
        if metrics:
            _print_metrics("Final ensemble metrics:", metrics)
        else:
            print("Evaluation completed but no final metrics were produced.")
